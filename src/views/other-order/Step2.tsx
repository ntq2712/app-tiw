...
const OtherOrderStep2: FC<TOtherOrderStep2> = props => {
                  ...
                  <View style={[styles.itemWrapper, {marginTop: index > 0 ? 16 : 0}]}>
                    <View style={styles.itemHeader}>
                      <View style={styles.productIndex}>
                        <Text style={{fontFamily: fonts.Semibold, color: '#fff'}}>Sản phẩm: {index + 1}</Text>
                      </View>
                    ...
                    <View style={[styles.inputWrapper, {marginTop: 16, width: '100%'}]}>
                      <TheInput
                        ...
                        onBlur={text => setItemValues(index, 'ProductLink', text)}
                        placeholder="Link sản phẩm"
                      />
                      <TouchableOpacity
                        onPress={async () => {
                          const clipText = await Clipboard.getString()
                          setItemValues(index, 'ProductLink', clipText)
                        }}
                        activeOpacity={0.7}
                    ...
                    <View style={{flexDirection: 'row', alignItems: 'center', width: '100%'}}>
                      <View style={[styles.inputWrapper, {marginTop: 16, flex: 1}]}>
                        <TheInput
                          defaultValue={item?.ProductVariable}
                          onBlur={text => setItemValues(index, 'ProductVariable', text)}
                          placeholder="Màu, size"
                        />
                        <TouchableOpacity
                          onPress={async () => {
                            const clipText = await Clipboard.getString()
                            setItemValues(index, 'ProductVariable', clipText)
                          }}
                          activeOpacity={0.7}
                          style={styles.btnPaste}>
                          <Icon type="FontAwesome5" color="#2196F3" size={16} name="paste" />
                    ...
                    <View style={[styles.inputWrapper, {marginTop: 16}]}>
                      <TheInput
                        ...
                        onBlur={text => setItemValues(index, 'Note', text)}
                        placeholder="Ghi chú"
                      />
                    </View>
                  </View>
...
export default OtherOrderStep2
